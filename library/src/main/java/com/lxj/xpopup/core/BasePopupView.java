...
public abstract class BasePopupView extends FrameLayout implements LifecycleObserver, LifecycleOwner,
    ...
    private static Integer orientation = null;// 屏幕方向，默认不指定，当屏幕方向变化时，应该对measure的width和height进行判断
    private final BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            System.out.println(" ------------- onReceive orientation = " + orientation);
        }
    };
...
//        wm.getDefaultDisplay().getSize(point);
        ...
        int width = activityContent.getWidth();
        int height = activityContent.getHeight();
        if (orientation != null) {
            int w = width;
            if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
                // 横屏时宽比高大
                width = Math.max(w, height);
                height = Math.min(w, height);
            } else {
                // 竖屏时宽比高小
                width = Math.min(w, height);
                height = Math.max(w, height);
            }
        }
        if (params == null) {
...
//                    addOnUnhandledKeyListener(et);
    ...
    protected void onCreate() {
        final Activity activity = getActivity();
        if (activity != null) {
            activity.registerReceiver(broadcastReceiver, new IntentFilter(Intent.ACTION_CONFIGURATION_CHANGED));
        }
    }
